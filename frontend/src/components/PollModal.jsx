      ...
      <div className="modal-content" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="modal-body">
          ...
          <div className="poll-settings" style={{ marginTop: 12, display: "flex", gap: 16 }}>
            ...
            <label><input type="checkbox" checked={isMultipleChoice} onChange={(e) => setIsMultipleChoice(e.target.checked)} /> Multiple choice</label>
          </div>
          <button className="btn-primary" onClick={handleCreate} disabled={sending || !question.trim() || options.some((o) => !o.trim())} style={{ marginTop: 16 }}>
            {sending ? <Loader2 size={16} className="spin" /> : "Create Poll"}
          </button>
        </div>
      </div>
    </div>
