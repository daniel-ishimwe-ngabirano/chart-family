      ...
      <div className="modal-content forward-modal" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="modal-body">
          <div className="search-bar">
            <Search size={18} />
            <input placeholder="Search chats..." value={search} onChange={(e) => setSearch(e.target.value)} autoFocus />
          </div>
          <div className="forward-list">
            {filtered.map((c) => {
              const name = c.isGroup ? c.groupName : c.members?.find((m) => m.user?.id !== authUser.id)?.user?.fullName || "Unknown";
              return (
                <div
                  key={c.id}
          ...
          <button className="btn-primary" onClick={handleForward} disabled={!selected || sending} style={{ marginTop: 12 }}>
            {sending ? "Forwarding..." : <><Send size={16} /> Forward Here</>}
          </button>
        </div>
      </div>
    </div>
